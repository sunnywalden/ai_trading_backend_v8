...
async def test_ai_config():
    ...
    print(f"  Models (fallback order): {service.models}")
        ...
        try:
            test_data = {
                ...
                "momentum": {
                    ...
                    "macd": {"value": 0.5, "signal_line": 0.3, "histogram": 0.2, "status": "BULLISH"}
                }
            }
            ...
            if summary:
                print(f"  ✓ GPT call successful")
                print(f"  Response preview: {summary[:100]}...")
            else:
                print(f"  ✗ GPT call failed, using rule-based fallback")
                
        except Exception as e:
            print(f"  ✗ Error: {str(e)}")
    else:
        print(f"  ⊗ Client not initialized, skipping test")
