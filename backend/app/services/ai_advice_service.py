...
class AiAdviceService:
    ...
    async def search_symbols(self, query: str) -> SymbolSearchResponse:
        ...
        try:
            broker_client = make_option_broker_client()
            positions = await broker_client.list_underlying_positions(await broker_client.get_account_id())
            for p in positions:
    ...
    async def analyze_stock_kline(self, symbol: str) -> KlineAnalysisResponse:
        ...
        if not client:
            return KlineAnalysisResponse(
                symbol=symbol, prediction="AI 模块未启用", suggestion="请配置 OpenAI API Key",
                direction="NEUTRAL", action="HOLD", details="后端未配置有效的 OpenAI 秘钥。"
            )
