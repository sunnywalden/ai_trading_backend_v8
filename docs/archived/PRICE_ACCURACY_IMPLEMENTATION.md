## 价格准确性与市价单模式 - 修改总结

### 🎯 核心需求

**用户要求**：
> 所有价格、数量相关字段，必须保证值的实时性、准确性，否则返回对应错误。如果无法做到，获取不到当前价格就以市价成交方式下单，且不设置对应的止盈、止损价位。

---

### ✅ 已完成修改

#### 1. 后端修改 (3个文件)

##### 1.1 `app/services/quick_trade_service.py`

**修改内容**：

1. **价格获取逻辑** - 确保准确性
   ```python
   # 修改前: 返回默认值 100.0
   async def _get_current_price(self, symbol: str) -> float:
       price = await self.market_data.get_current_price(symbol)
       return price if price and price > 0 else 100.0
   
   # 修改后: 抛出异常
   async def _get_current_price(self, symbol: str) -> float:
       price = await self.market_data.get_current_price(symbol)
       if not price or price <= 0:
           raise ValueError(f"无法获取 {symbol} 的准确价格")
       return price
   ```

2. **账户权益获取** - 确保准确性
   ```python
   # 修改前: 返回默认值 1000000.0
   async def _get_account_equity(self) -> float:
       equity = await self.broker.get_account_equity(account_id)
       return equity if equity > 0 else 1000000.0
   
   # 修改后: 抛出异常
   async def _get_account_equity(self) -> float:
       equity = await self.broker.get_account_equity(account_id)
       if not equity or equity <= 0:
           raise ValueError(f"账户权益数据异常: {equity}")
       return equity
   ```

3. **预览逻辑支持双模式**
   - **模式A - 限价单**：有准确价格，正常计算数量、止盈止损
   - **模式B - 市价单**：无价格时，不计算数量、不设置止盈止损
   
   ```python
   async def preview_quick_trade(...) -> Dict[str, Any]:
       # 尝试获取价格
       try:
           current_price = await self._get_current_price(symbol)
           # 限价单模式
           return {
               "order_mode": "LIMIT",
               "price_available": True,
               "current_price": 670.49,
               "calculated_quantity": 2135,
               "calculated_stop_loss": 637.00,
               "calculated_take_profit": 771.00,
               ...
           }
       except Exception:
           # 市价单模式
           return {
               "order_mode": "MARKET",
               "price_available": False,
               "current_price": None,
               "calculated_quantity": None,
               "calculated_stop_loss": None,
               "calculated_take_profit": None,
               "warning": "无法获取实时价格，将以市价单执行，不设置止盈止损"
           }
   ```

4. **信号创建支持市价单**
   ```python
   async def _create_signal_from_preview(...) -> TradingSignal:
       signal = TradingSignal(
           suggested_quantity=preview.get("calculated_quantity"),  # 可为 None
           suggested_price=preview.get("current_price"),  # 可为 None
           stop_loss=preview.get("calculated_stop_loss"),  # 可为 None
           take_profit=preview.get("calculated_take_profit"),  # 可为 None
           notes=f"{notes} | Order Mode: {order_mode}"
       )
   ```

##### 1.2 `docs/PRICE_ACCURACY_FIX.md`
- 详细测试文档
- 包含场景说明、验收标准、部署步骤

---

#### 2. 前端修改 (1个文件)

##### 2.1 `frontend/src/components/QuickTradeDialog.vue`

**修改内容**：

1. **接口类型定义**
   ```typescript
   interface QuickTradePreview {
     order_mode?: string;        // 'LIMIT' | 'MARKET'
     price_available?: boolean;  // 价格是否可用
     current_price: number | null;
     calculated_quantity: number | null;
     calculated_stop_loss: number | null;
     calculated_take_profit: number | null;
     warning?: string;           // 市价单警告
   }
   ```

2. **市价单警告横幅**
   ```vue
   <div v-if="!preview.price_available" class="warning-banner">
     <span class="warning-icon">⚠️</span>
     <div class="warning-content">
       <strong>市价单模式</strong>
       <p>{{ preview.warning }}</p>
     </div>
   </div>
   ```

3. **价格显示处理**
   ```vue
   <!-- 有价格时显示具体价格 -->
   <span v-if="preview.current_price" class="value price-value">
     ${{ preview.current_price.toFixed(2) }}
   </span>
   <!-- 无价格时显示"市价" -->
   <span v-else class="value price-value market-price">
     市价
   </span>
   ```

4. **输入框禁用处理**
   ```vue
   <!-- 市价单模式：禁用数量输入 -->
   <input v-if="preview.calculated_quantity" type="number" ... />
   <input v-else type="text" value="市价成交" disabled class="input-field disabled" />
   
   <!-- 市价单模式：禁用止损输入 -->
   <input v-if="preview.calculated_stop_loss !== null" type="number" ... />
   <input v-else type="text" value="不设置" disabled class="input-field disabled" />
   ```

5. **警告横幅样式**
   ```css
   .warning-banner {
     background: linear-gradient(135deg, rgba(255, 152, 0, 0.15) 0%, rgba(255, 193, 7, 0.08) 100%);
     border: 1px solid rgba(255, 152, 0, 0.4);
     animation: pulse-warning 2s ease-in-out infinite;
   }
   ```

---

### 📊 数据流对比

#### 之前的流程（存在问题）
```
用户点击快捷交易
  ↓
调用 preview API
  ↓
后端尝试获取价格
  ├─ 成功: 返回真实价格
  └─ 失败: 返回默认值 100.0 ❌
  ↓
前端显示价格 (可能是错误的 $100.00)
  ↓
用户确认交易 (基于错误价格) ❌
```

#### 修改后的流程（正确）
```
用户点击快捷交易
  ↓
调用 preview API
  ↓
后端尝试获取价格
  ├─ 成功: 限价单模式
  │   ✅ 返回真实价格 $670.49
  │   ✅ 计算精确数量 2135股
  │   ✅ 设置止盈止损
  │
  └─ 失败: 市价单模式
      ✅ 返回 price_available: false
      ✅ 返回警告信息
      ✅ 不计算数量、不设置止盈止损
  ↓
前端根据 order_mode 显示
  ├─ LIMIT: 显示价格、数量、止盈止损
  └─ MARKET: 显示警告横幅、禁用部分输入
  ↓
用户明确知道当前模式
  ├─ 限价单: 基于准确数据确认
  └─ 市价单: 知道将以市价成交 ✅
```

---

### 🧪 测试验证

#### 测试场景1: 价格获取成功（限价单）
```bash
curl http://localhost:8000/api/v1/strategy-runs/{run_id}/assets/META/preview
```

**预期响应**:
```json
{
  "order_mode": "LIMIT",
  "price_available": true,
  "current_price": 670.49,
  "calculated_quantity": 2135,
  "calculated_stop_loss": 637.00,
  "calculated_take_profit": 771.00
}
```

**前端显示**:
- 当前价格: $670.49 ✅
- 交易数量: 2135 股 ✅
- 止损价: $637.00 ✅
- 止盈价: $771.00 ✅
- 无警告横幅 ✅

---

#### 测试场景2: 价格获取失败（市价单）
```bash
# 模拟: Tiger API 未配置 && Yahoo Finance 失败
```

**预期响应**:
```json
{
  "order_mode": "MARKET",
  "price_available": false,
  "current_price": null,
  "calculated_quantity": null,
  "calculated_stop_loss": null,
  "calculated_take_profit": null,
  "warning": "无法获取实时价格，将以市价单执行，不设置止盈止损"
}
```

**前端显示**:
- ⚠️ 警告横幅（橙色脉冲动画） ✅
- 当前价格: 市价 ✅
- 交易数量: 市价成交（禁用输入框） ✅
- 止损价: 不设置（禁用输入框） ✅
- 止盈价: 不设置（禁用输入框） ✅

---

### 🎨 UI 效果预览

#### 限价单模式
```
┌─────────────────────────────────────┐
│ ⚡ 快捷交易                          │
│ META                                │
├─────────────────────────────────────┤
│ 市场信息                            │
│ 当前价格        信号强度             │
│ $670.49         ████████░░ 90.6     │
│                                     │
│ 交易参数                            │
│ 交易数量: [2135        ] 股         │
│ 止损价:   [637.00      ]           │
│ 止盈价:   [771.00      ]           │
│                                     │
│ [取消] [创建计划] [限价单] [市价单] │
└─────────────────────────────────────┘
```

#### 市价单模式
```
┌─────────────────────────────────────┐
│ ⚡ 快捷交易                          │
│ META                                │
├─────────────────────────────────────┤
│ ⚠️  市价单模式                      │
│     无法获取实时价格，将以市价单执行 │
│     不设置止盈止损                   │
├─────────────────────────────────────┤
│ 市场信息                            │
│ 当前价格        信号强度             │
│ 市价            ████████░░ 90.6     │
│                                     │
│ 交易参数                            │
│ 交易数量: [市价成交    ] 股 (禁用)  │
│ 止损价:   [不设置      ] (禁用)     │
│ 止盈价:   [不设置      ] (禁用)     │
│                                     │
│ [取消] [创建计划] [限价单] [市价单] │
└─────────────────────────────────────┘
```

---

### 📋 部署检查清单

- [ ] 后端代码已修改: `quick_trade_service.py`
- [ ] 前端代码已修改: `QuickTradeDialog.vue`
- [ ] 数据库字段允许 NULL: `suggested_price`, `suggested_quantity`, `stop_loss`, `take_profit`
- [ ] 测试限价单模式：价格获取成功场景
- [ ] 测试市价单模式：价格获取失败场景
- [ ] 测试批量交易：部分价格失败场景
- [ ] 检查日志输出：价格获取来源标记
- [ ] 检查信号记录：Order Mode 标记
- [ ] 前端 UI 验证：警告横幅显示
- [ ] 前端 UI 验证：禁用输入框样式

---

### 🚀 投产信息

**修改时间**: 2026-02-12  
**版本号**: v3.0.2+price-accuracy-fix  
**修改人**: AI Assistant  
**测试状态**: 待测试  
**影响范围**: 
- 快捷交易功能（单资产、批量）
- 交易预览接口
- 交易信号创建
- 前端交易对话框UI

**回滚方案**:
```bash
git revert HEAD
# 或恢复到 v3.0.2 版本
git checkout v3.0.2
```

---

### 💡 后续建议

1. **监控报警**: 监控市价单使用频率，如果过高说明价格服务有问题
2. **用户提示优化**: 市价单风险提示更明确
3. **历史价格回退**: 考虑使用最近的历史价格（带时间戳）作为参考
4. **价格数据源配置**: 支持多数据源优先级配置
